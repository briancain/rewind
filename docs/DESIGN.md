...
> per-source-IP rate-based rule + the AWS managed IP-reputation, common, and known-bad-inputs rule
...
- **`shallow`** (hourly, read-only): health of every service + public feed + a search query + a **region-routing DNS check** — it resolves a latency-routed host and this region's region-pinned host and asserts they hit the same ALB, proving Route 53 kept this region's traffic in-region.
